...
export default function Result(){
  ...
  return<div className="page-container"><button onClick={()=>nav("/")} className="text-white/15 text-xs mb-4">{t("howto.back")}</button><h2 className="text-lg font-black mb-3">{t("result.matchNumber")}{game.id}</h2><div className="card"><div className="flex justify-between mb-2"><span className="text-white/20 text-xs">{t("result.base")}</span><span className="font-mono text-sm">${parseFloat(game.base_price).toLocaleString()}</span></div><div className="flex justify-between"><span className="text-white/20 text-xs">{t("result.settlement")}</span><span className={`font-mono font-bold text-sm ${dir==="up"?"text-emerald-400":"text-rose-400"}`}>${parseFloat(game.settlement_price).toLocaleString()}</span></div></div></div>;
}
