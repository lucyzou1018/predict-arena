export function PaymentModal({
  ...
  return<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-xl">
    <div className={`max-w-sm w-full mx-4 animate-slideUp rounded-2xl border shadow-2xl p-6 ${isWaiting?"border-emerald-500/20 bg-gradient-to-br from-[#102016] via-[#131a14] to-[#0f130f] shadow-emerald-950/30":"border-amber-500/20 bg-gradient-to-br from-[#22160f] via-[#17110d] to-[#120d0a] shadow-orange-900/20"}`}>
      <div className="text-center mb-4">
        <div className={`w-14 h-14 mx-auto mb-3 rounded-2xl border flex items-center justify-center text-2xl ${isWaiting?"bg-gradient-to-br from-emerald-500/15 to-teal-500/15 border-emerald-500/20":"bg-gradient-to-br from-amber-500/15 to-orange-500/15 border-orange-500/15 animate-float"}`}>{isWaiting?"✅":"💰"}</div>
      ...
      {amount&&<div className={`border rounded-xl p-4 mb-3 text-center ${isWaiting?"bg-gradient-to-br from-emerald-500/[0.08] to-teal-500/[0.05] border-emerald-500/20":"bg-gradient-to-br from-amber-500/[0.08] to-orange-500/[0.06] border-amber-500/20"}`}>
        ...
        <p className="text-white/20 text-[10px] mt-1">{isWaiting?"Your payment is locked in for this round":"Platform fee included"}</p>
      </div>}
      {totalCount>0&&<div className={`rounded-xl border p-3 mb-3 ${isWaiting?"bg-emerald-500/[0.06] border-emerald-500/15":"bg-white/[0.03] border-white/[0.06]"}`}>
