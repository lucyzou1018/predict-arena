...
export function PredictButtons({onPredict,disabled,myPrediction,attention=false}){
  ...
  return<div className="grid grid-cols-2 gap-3 justify-center my-1.5">
    <button
      type="button"
      aria-label={t("game.chooseLongAria")}
    ...
    >
      ...
      {!locked&&<span className="prediction-choice-cta">{t("game.choose")}</span>}
      {myPrediction==="up"&&<span className="prediction-choice-selected">{t("game.selected")}</span>}
      <span className={`flex h-full flex-col items-center justify-center gap-1.5 px-3.5 ${locked?"pt-8 pb-3":"pt-8 pb-3"}`}>
        ...
        <span className={`text-[10px] font-mono uppercase tracking-[0.24em] ${myPrediction==="up"?"text-emerald-100/92":"text-emerald-100/78 group-hover:text-white"}`}>{t("game.aboveBase")}</span>
        <span className="text-[1.82rem] font-black tracking-[-0.05em] leading-none">{t("game.long")}</span>
        <span className={`text-[11px] font-semibold ${myPrediction==="up"?"text-emerald-100/82":"text-white/58 group-hover:text-white/76"}`}>{t("game.btcHigher")}</span>
      </span>
    </button>
    <button
      type="button"
      aria-label={t("game.chooseShortAria")}
    ...
    >
      ...
      {!locked&&<span className="prediction-choice-cta">{t("game.choose")}</span>}
      {myPrediction==="down"&&<span className="prediction-choice-selected">{t("game.selected")}</span>}
      <span className={`flex h-full flex-col items-center justify-center gap-1.5 px-3.5 ${locked?"pt-8 pb-3":"pt-8 pb-3"}`}>
        ...
        <span className={`text-[10px] font-mono uppercase tracking-[0.24em] ${myPrediction==="down"?"text-rose-100/92":"text-rose-100/78 group-hover:text-white"}`}>{t("game.belowBase")}</span>
        <span className="text-[1.82rem] font-black tracking-[-0.05em] leading-none">{t("game.short")}</span>
        <span className={`text-[11px] font-semibold ${myPrediction==="down"?"text-rose-100/82":"text-white/58 group-hover:text-white/76"}`}>{t("game.btcLower")}</span>
      </span>
    </button>
  </div>;
}
